...
import (
	...
	"kudejen/src/internal/app/services"
...
func AddMiddlewares(server *server.Server) {
	logger := log.NewLogfmtLogger(os.Stderr)
	rpcLogger := log.With(logger, "service", "gRPC/server", "component", "kudeJen")
	...
	resourceService := &services.ResourceGeneratorService{
		Server: server,
	}
	resource_service.RegisterResourceGeneratorServer(grpcSrv, resourceService)
	srvMetrics.InitializeMetrics(grpcSrv)

	g := &run.Group{}
	g.Add(func() error {
		l, err := net.Listen("tcp", configs.AppConfig.Application.GRPCPort)
		if err != nil {
			return err
		}
	...
	httpSrv := &http.Server{Addr: configs.AppConfig.Application.HTTPPort}
	g.Add(func() error {
		m := http.NewServeMux()
